...
public class UIInventoryBar : MonoBehaviour
{
    ...
    public void ClearHighlightOnInventorySlots()
    {
        ...
        {
            // loop through inventory slots and clear highlight sprite
            ...
            if (inventorySlot.Length > 0 && inventoryList.Count > 0)
            {
                ...
                for (int i = 0; i < inventorySlot.Length; i++)
                {
                    if (i < inventoryList.Count)
                    {
                        ...
                        // ItemDetails itemDetails = InventoryManage.Instance.itemList.itemDetails.Find(x => x.itemCode == itemCode);
    ...
    /// <summary>
    ///  Set the selected hightlight if set on all inventory item positions
    /// </summary>
    public void SetHighlightedInventorySlots()
    {
        if (inventorySlot.Length > 0)
        {
            // loop through inventory slots and clear hightlight sprites
            for (int i = 0; i < inventorySlot.Length; i++)
            {
                SetHighlightedInventorySlots(i);
            }
